...
class Datapreprocess ():
    ...
    def transform_hit_e(self,hit_energies):
    ...
    # Min max for z layer
    def transform_hit_z(self,z_,maxz_,minz_):
        #maxz_ = np.max(z_)
        #minz_ = np.min(z_)
        z_ = (z_ - minz_) / (maxz_ - minz_)
        return z_

    def transform_hit_z_1(self,z_):
        max = np.max(z_)
        min = np.min(z_)
        return (max,min)
    ...
    def transform_data(self):
        ...
        # For each file
        for infile in os.listdir(self.input):
            if fnmatch.fnmatch(infile, 'dataset_2_tensor_no_pedding_euclidian_nentry424To564.pt'):
                filename = os.path.join(self.input,infile)
                ofile = infile.replace("tensor_no_pedding_euclidian", "padded_diffenrent_nontransformed" )
                opath = './'
                odir = 'test'
                ...
                print(incident_energies)
                incident_energies = self.transform_incident_energy(incident_energies)
                ...
                max,min = self.transform_hit_z_1(Z_collect_flat)
                for showers in custom_data.data:
                    pad_hits = max_nhits-showers.shape[0]
                    if showers.shape[0] == 0:
                        print(f'incident e: {incident_energies[shower_count]} with {showers.shape[0]} hits')
                    ...
                    self.E_original.append(showers[:,0].cpu()*GeV)
                    self.X_original.append(showers[:,1].cpu())
                    self.Y_original.append(showers[:,2].cpu())
                    self.Z_original.append(showers[:,3].cpu())

                    
                    
                    E_ = self.transform_hit_e(E_)
                    X_ = self.transform_hit_xy(X_)
                    Y_ = self.transform_hit_xy(Y_)
                    Z_ = self.transform_hit_z(Z_,max,min)
                    ...
                    self.E_no_pedding.append(E_)
                    self.X_no_pedding.append(X_)
                    self.Y_no_pedding.append(Y_)
                    self.Z_no_pedding.append(Z_)
                    padded_shower = F.pad(input = shower_data_transformed, pad=(0,0,0,pad_hits), mode='constant', value=0.0)
                    self.E_pedding.append(padded_shower[0])
                    self.X_pedding.append(padded_shower[1])
                    self.Y_pedding.append(padded_shower[2])
                    self.Z_pedding.append(padded_shower[3])
                    # normal padding
    ...
    def draw_distribution(self,output_directory,transform = 'default'):

        '''
        the transform here should contain the variable that you want to inverse-transform,
        for example transform = 'exy', then it will inverse-tranform e,x,y but not z
        ''' 

        all_x = self.X_original
        all_y = self.Y_original
        all_z = self.Z_original
        all_e = self.E_original

        

        if 'x' in transform:
            all_x = self.X_no_pedding
        if 'y' in transform:
            all_y = self.Y_no_pedding
        if 'z' in transform:
            all_z = self.Z_no_pedding
        if 'e' in transform:
            all_e = self.E_no_pedding

        all_e = [item for sublist in all_e for item in sublist]
        all_x = [item for sublist in all_x for item in sublist]
        all_y = [item for sublist in all_y for item in sublist]
        all_z = [item for sublist in all_z for item in sublist]
        ...
        fig_name = os.path.join(output_directory, 'preprocess.png')
